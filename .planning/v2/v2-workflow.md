...
```
/gsd-new-project --auto @.planning/v2/v2.md
```
...
Consult `.planning/v2/v2-issue-map.md` for the exact branch name for this issue — do not invent or abbreviate it.
...
`.planning/v2/v2.md` as canonical context (use the table of spec documents to
...
- Plans reference the appropriate spec documents (see `.planning/v2/v2.md` for the index) and `.planning/codebase/` for context
- Confirm issue number, title, and scope against `.planning/v2/v2-issue-map.md` before writing the plan
...
| Document | Purpose |
|---|---|
| `.planning/v2/v2.md` | Top-level index — overview, disk layout, motivation, and table of all spec documents |
| `.planning/v2/v2-strategy.md` | Branching hierarchy, sequencing, and Active State |
| `.planning/v2/v2-issue-map.md` | **Authoritative issue and milestone map** — exact GitHub issue numbers, titles, branch names, and dependencies for all 62 issues across 12 milestones. Read this before branching or planning any issue. |
| `.planning/v2/v2-schema.md` | Config file model and all schema blocks: `[[workspace]]`, `[[repo]]`, `[[remote]]`, `[[sync_pair]]`, `[[workstream]]`, `.gitw-stream` manifest |
| `.planning/v2/v2-commands.md` | Full command tree, cut list from v1, per-command specifications, status output format |
| `.planning/v2/v2-remote-management.md` | `git w sync` fan-out, `git w remote` subcommand, workstream push protection, `pre-push` hook, state file, implementation notes |
| `.planning/v2/v2-infra-patterns.md` | Pattern A (branch-per-env) and Pattern B (folder-per-env) with all creation examples |
| `.planning/v2/v2-agent-interop.md` | Three-level `AGENTS.md` strategy, `git w agent context --json`, GSD interop specifics |
| `.planning/v2/v2-migration.md` | `git w migrate` spec, `pkg/migrate` package, v1 breaking changes summary |
| `.planning/v2/v2-milestones.md` | All 12 milestone scope descriptions, resolved design decisions, deferred items |
