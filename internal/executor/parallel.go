...
func RunParallel(repos []repo.Repo, args []string, opts ExecOptions) []ExecResult {
	...
	return runConcurrent(ctx, repos, args, workers)
}
...
func maxWorkers(opts ExecOptions) int {
...
func runConcurrent(ctx context.Context, repos []repo.Repo, args []string, workers int) []ExecResult {
	results := make([]ExecResult, len(repos))
	...
	for i, r := range repos {
		...
		go func(idx int, r repo.Repo) {
			defer wg.Done()
			defer func() { <-sem }()
			results[idx] = runAsync(ctx, r, args)
		}(i, r)
	}
	wg.Wait()
